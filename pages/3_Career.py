...
from ui.panel import (wkey, render_save_load, page_header, section, metric_row,
                      money, pct, integer, number, toggle, choice, fmt_money,
                      fmt_pct, esc, md_money, render_findings, mark_dirty,
                      invalidate)
...
with section("Career timeline",
             "Drag each marker to the year of service you expect it. Defaults "
             "are service averages — officer timing follows DOPMA zones and is "
             "fairly predictable; enlisted timing swings widely by branch and "
             "specialty."):

    ctl, viz = st.columns([1, 2.1])
...
with section("Planned moves",
    ...
    mv1, mv2 = st.columns([1.1, 2])
    with mv1:
        rows = [{"At YOS": mv.at_years_of_service, "ZIP": mv.destination_zip,
                 "Label": mv.destination_label,
                 "Into quarters": mv.into_government_housing} for mv in t.moves]
        if not rows:
            rows = [{"At YOS": min(START + 3, END), "ZIP": "", "Label": "",
                     "Into quarters": False}]
    ...
    with mv2:
        cz1, cz2, cz3 = st.columns([1, 1, 0.8])
        with cz1:
...
if rows:
    with section("Projected household pay"):
        ...
        st.caption("BAH is not inflated forward — it is a market rate, and "
                   "guessing its trajectory adds error rather than removing it.")
